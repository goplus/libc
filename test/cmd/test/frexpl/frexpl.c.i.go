...
func _cgo_main() int32 {
	...
	for i = int32(0); uint64(i) < 22; i++ {
		p = (*struct_l_li)(unsafe.Pointer(uintptr(unsafe.Pointer((*struct_l_li)(unsafe.Pointer(&t_cgo1_frexpl)))) + uintptr(i)*52))
		if p.r < int32(0) {
			continue
		}
		libc.Fesetround(p.r)
