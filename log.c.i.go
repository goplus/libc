...
func _cgos_top16_log(x float64) uint32 {
...
func Log(x float64) float64 {
	...
	top = _cgos_top16_log(x)
	if func() int64 {
		if ix-*(*uint64)(unsafe.Pointer(&_cgoz_22_log{1.0 - 0.0625})) < *(*uint64)(unsafe.Pointer(&_cgoz_23_log{1.0 + 0.064697265625}))-*(*uint64)(unsafe.Pointer(&_cgoz_24_log{1.0 - 0.0625})) {
			return 1
		} else {
			return 0
		}
		...
		if int32(1) != 0 && func() int64 {
			if ix == *(*uint64)(unsafe.Pointer(&_cgoz_25_log{1.0})) {
				return 1
			} else {
				return 0
			}
	...
	if func() int64 {
		...
		ix = *(*uint64)(unsafe.Pointer(&_cgoz_27_log{x * 4503599627370496.0}))
