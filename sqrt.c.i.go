...
func mul32_cgo18_sqrt(a uint32, b uint32) uint32 {
	return uint32(uint64(a) * uint64(b) >> int32(32))
}
func mul64_cgo19_sqrt(a uint64, b uint64) uint64 {
...
func Sqrt(x float64) float64 {
	...
	var top uint64
	var m uint64
	ix = *(*uint64)(unsafe.Pointer(&_cgoz_20_sqrt{x}))
	top = ix >> int32(52)
	if func() int64 {
		...
		ix = *(*uint64)(unsafe.Pointer(&_cgoz_21_sqrt{x * 4503599627370496}))
		top = ix >> int32(52)
		top -= uint64(52)
	}
	...
	s = uint64(mul32_cgo18_sqrt(uint32(m>>int32(32)), uint32(r)))
	d = uint64(mul32_cgo18_sqrt(uint32(s), uint32(r)))
	u = three_cgo22_sqrt - d
	r = uint64(mul32_cgo18_sqrt(uint32(r), uint32(u)) << int32(1))
	s = uint64(mul32_cgo18_sqrt(uint32(s), uint32(u)) << int32(1))
	d = uint64(mul32_cgo18_sqrt(uint32(s), uint32(r)))
	u = three_cgo22_sqrt - d
	r = uint64(mul32_cgo18_sqrt(uint32(r), uint32(u)) << int32(1))
	r = r << int32(32)
	s = mul64_cgo19_sqrt(m, r)
	d = mul64_cgo19_sqrt(s, r)
	u = three_cgo22_sqrt<<int32(32) - d
	s = mul64_cgo19_sqrt(s, u)
	...
	s |= top << int32(52)
	y = *(*float64)(unsafe.Pointer(&_cgoz_23_sqrt{s}))
	if int32(1) != 0 {
		...
		t = *(*float64)(unsafe.Pointer(&_cgoz_24_sqrt{tiny}))
...
var three_cgo22_sqrt uint64 = uint64(3221225472)

type _cgoz_23_sqrt struct {
	_i uint64
}
type _cgoz_24_sqrt struct {
	_i uint64
}
