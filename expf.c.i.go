...
func top12_cgo18_expf(x float32) uint32 {
...
func Expf(x float32) float32 {
	...
	var s float64
	xd = float64(x)
	abstop = top12_cgo18_expf(x) & uint32(2047)
	if func() int64 {
		if abstop >= top12_cgo18_expf(88) {
			return 1
		} else {
			return 0
		}
	}() == int64(0) {
		if *(*uint32)(unsafe.Pointer(&_cgoz_20_expf{x})) == *(*uint32)(unsafe.Pointer(&_cgoz_21_expf{-X__builtin_inff()})) {
			return float32(0)
		}
		if abstop >= top12_cgo18_expf(X__builtin_inff()) {
			return x + x
		}
		if x > 88.7228317 {
	...
	ki = *(*uint64)(unsafe.Pointer(&_cgoz_22_expf{kd}))
	...
	t += ki << 47
	s = *(*float64)(unsafe.Pointer(&_cgoz_23_expf{t}))
