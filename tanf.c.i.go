...
var t1pio2_cgo18_tanf float64 = float64(int32(1)) * 1.5707963267948966
var t2pio2_cgo19_tanf float64 = float64(int32(2)) * 1.5707963267948966
var t3pio2_cgo20_tanf float64 = float64(int32(3)) * 1.5707963267948966
var t4pio2_cgo21_tanf float64 = float64(int32(4)) * 1.5707963267948966
...
func Tanf(x float32) float32 {
	...
	var sign uint32
	for {
		ix = *(*uint32)(unsafe.Pointer(&_cgoz_22_tanf{x}))
		if true {
			break
		}
	}
	...
	if ix <= uint32(1081824209) {
		if ix <= uint32(1075235811) {
			return __tandf(func() float64 {
				if sign != 0 {
					return float64(x) + t1pio2_cgo18_tanf
				} else {
					return float64(x) - t1pio2_cgo18_tanf
				}
			}(), int32(1))
			...
			return __tandf(func() float64 {
				if sign != 0 {
					return float64(x) + t2pio2_cgo19_tanf
				} else {
					return float64(x) - t2pio2_cgo19_tanf
				}
			}(), int32(0))
	...
	if ix <= uint32(1088565717) {
		if ix <= uint32(1085271519) {
			return __tandf(func() float64 {
				if sign != 0 {
					return float64(x) + t3pio2_cgo20_tanf
				} else {
					return float64(x) - t3pio2_cgo20_tanf
				}
			}(), int32(1))
			...
			return __tandf(func() float64 {
				if sign != 0 {
					return float64(x) + t4pio2_cgo21_tanf
				} else {
					return float64(x) - t4pio2_cgo21_tanf
				}
			}(), int32(0))
