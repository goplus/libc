...
func printf_core_cgo32_vfprintf(f *struct__IO_FILE, fmt *int8, ap *[]interface {
	...
	for {
		...
		for {
			...
			ps = st
			st = uint32(*(*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer((*uint8)(unsafe.Pointer(&*(*[58]uint8)(unsafe.Pointer(uintptr(unsafe.Pointer((*[58]uint8)(unsafe.Pointer(&states_cgo22_vfprintf)))) + uintptr(st)*58)))))) + uintptr(int32(*func() (_cgo_ret *int8) {
				_cgo_addr := &s
				_cgo_ret = *_cgo_addr
				*(*uintptr)(unsafe.Pointer(_cgo_addr))++
				return
